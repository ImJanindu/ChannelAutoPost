...
async def _(event):
    ...
    await event.reply(f"Hi `{ok.user.first_name}`!\n\nI am a channel auto-post bot. Read /help to know more!\n\nI can be used in only two channels (one user) at a time. Kindly deploy your own bot.\n\n[More bots](https://t.me/Infinity_BOTs)", buttons=[Button.url("Repo", url="https://github.com/ImJanindu/ChannelAutoPost"), Button.url("Support", url="https://t.me/Infinity_BOTs"), Button.url("Developer", url="https://t.me/ImJanindu")], link_preview=False)


@datgbot.on(events.NewMessage(pattern="/help"))
async def helpp(event):
    await event.reply("**Help**\n\nThis bot will send all new posts in one channel to the other channel (without forwarded tag).\nIt can be used only in two channels at a time, so kindly deploy your own bot from [here](https://github.com/ImJanindu/ChannelAutoPost).\n\nAdd me to both the channels and make me an admin in both, and all new messages would be autoposted on the linked channel!\n\nLiked the bot? Drop a ♥ to @ImJanindu :)")
